...
static enum ls_lightsense_mode_t _ls_lightsense_current_mode =
...
static enum ls_lightsense_level_t
_ls_lightsense_level_from_adc(uint32_t adc_reading) {
...
int ls_lightsense_read_adc(adc_atten_t attenuation) {
  ...
  esp_adc_cal_characterize(ADC_UNIT_1, attenuation, ADC_WIDTH_12Bit, 1100,
                           &adc_cal);
...
/**
 * @brief Set current mode if event can be queued; otherwise let it try again
 * next read
 *
 * @param mode
 */
...
#endif
  if (pdTRUE ==
      xQueueSendToBack(ls_event_queue, (void *)&lightsense_event, 0)) {
...
void ls_lightsense_read_task(void *pvParameter) {
  ...
  while (1) {
    ...
    ls_debug_printf("Light sense %dmV (raw=%d); level=%d\n", adc_reading,
                    _ls_lightsense_adc_raw, (u_int8_t)levels[level_index]);
    ...
    if (all_agree || ls_tapemode() == LS_TAPEMODE_SELFTEST) {
      ...
      case LS_LIGHTSENSE_LEVEL_NIGHT:
        if (ls_lightsense_current_mode() != LS_LIGHTSENSE_MODE_NIGHT ||
            (ls_state_current.func != ls_state_sleep &&
             ls_state_current.func != ls_state_settings_upper &&
             ls_state_current.func != ls_state_settings_lower &&
             ls_state_current.func != ls_state_settings_both)) {
