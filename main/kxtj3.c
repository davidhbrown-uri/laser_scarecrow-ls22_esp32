...
static esp_err_t kxtj3_write_reg_byte(uint8_t register_number, uint8_t data) {
  ...
  i2c_master_write_byte(cmd, kxtj3_addr << 1 | I2C_MASTER_WRITE,
...
static esp_err_t kxtj3_read_reg_uint8(uint8_t register_number, uint8_t *data) {
  ...
  i2c_master_write_byte(cmd, register_number, ACK_CHECK_EN); // RA (ACK)
  i2c_master_start(cmd);                                     // S
  i2c_master_write_byte(cmd, kxtj3_addr << 1 | I2C_MASTER_READ,
...
static esp_err_t kxtj3_read_reg_int16(uint8_t register_number, int16_t *data)
{
    ...
    i2c_master_start(cmd); // S
    i2c_master_write_byte(cmd, kxtj3_addr << 1 | I2C_MASTER_WRITE,
ACK_CHECK_EN); // SAD+W (ACK) i2c_master_write_byte(cmd, register_number,
ACK_CHECK_EN); // RA (ACK) i2c_master_start(cmd); // S
    i2c_master_write_byte(cmd, kxtj3_addr << 1 | I2C_MASTER_READ, ACK_CHECK_EN);
// SAD+R (ACK) i2c_master_read_byte(cmd, &data_l, I2C_MASTER_ACK); // (DATA) ACK
...
static esp_err_t kxtj3_software_reset_sequence(void) {
  ...
  // 3.1.3 Software Reset Timing Diagram says to wait for power-on time before
  // attempting reset 30ms maximum power-on time...
  vTaskDelay(1);
  // 3.1.2 (a)
  ...
  if (ESP_FAIL == response) { //"slave hasn't ACK the transfer"
    ...
    if (ESP_FAIL == response) {
      printf("KXTJ3 did not respond on either KXTJ3_ADDR or "
             "KXTJ3_ADDR_FLIPPED.\n");
      kxtj3_fail();
      return ESP_FAIL;
    }
...
void kxtj3_begin(void) {
  ...
  // reset value of CTRL_REG1 is 0x00: standby mode, but I suppose there's no
  // harm in making sure.
