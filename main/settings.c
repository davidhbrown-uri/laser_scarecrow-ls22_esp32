...
static BaseType_t _ls_settings_mode;
...
static BaseType_t _ls_settings_servo_pulse_delta,
    _ls_settings_servo_random_pause_ms, _ls_settings_servo_sweep_pause_ms;
...
void ls_settings_set_defaults(void) {
  ...
  ls_settings_set_light_threshold_off((int)((int[]){
      LS_LIGHTSENSE_THRESHOLDS_OFF_MV})[LS_LIGHTSENSE_THRESHOLD_DEFAULT]);
...
void ls_settings_reset_for_current_spinmode(void){
  ...
  switch(ls_spinmode()) {
      ...
      case LS_SPINMODE_HOP_FARTHER:
          ...
          ls_settings_set_servo_top(LS_SERVO_US_0DEG); // not quite the top
          ls_settings_set_servo_bottom(50);          // half way 
          ls_settings_set_mode(ls_spinmode());
          ls_settings_save();
      break;
...
static void _ls_settings_open_nvs(void) {
  ...
  // Open
  err = nvs_open(LS_SETTINGS_NVS_NAMESPACE, NVS_READWRITE,
                 &_ls_settings_nvs_handle);
...
static esp_err_t _ls_settings_read_from_nvs(char *name,
...
void ls_settings_read(void) {
  ...
  if (ESP_OK == _ls_settings_read_from_nvs(LS_SETTINGS_NVS_KEY_MODE,
                                           &nvs_value)) {
    ls_settings_set_mode(nvs_value);
  }
  ...
  if (ESP_OK == _ls_settings_read_from_nvs(LS_SETTINGS_NVS_KEY_SERVO_BOTTOM,
                                           &nvs_value)) {
    ls_settings_set_servo_bottom(nvs_value);
  }
  if (ESP_OK ==
      _ls_settings_read_from_nvs(LS_SETTINGS_NVS_KEY_SERVO_DELTA, &nvs_value)) {
    ls_settings_set_servo_pulse_delta(nvs_value);
  }
  if (ESP_OK == _ls_settings_read_from_nvs(LS_SETTINGS_NVS_KEY_LIGHTSENSE_OFF,
                                           &nvs_value)) {
    ls_settings_set_light_threshold_off(nvs_value);
  }
  if (ESP_OK == _ls_settings_read_from_nvs(LS_SETTINGS_NVS_KEY_LIGHTSENSE_ON,
                                           &nvs_value)) {
    ls_settings_set_light_threshold_on(nvs_value);
  }
  if (ESP_OK == _ls_settings_read_from_nvs(
                    LS_SETTINGS_NVS_KEY_SLEEP_LIGHT_ENABLE, &nvs_value)) {
...
void ls_settings_save(void) {
  ...
  if (ESP_OK == nvs_set_i32(_ls_settings_nvs_handle,
                            LS_SETTINGS_NVS_KEY_STEPPER_SPEED,
  ...
  if (ESP_OK == nvs_set_i32(_ls_settings_nvs_handle,
                            LS_SETTINGS_NVS_KEY_SERVO_BOTTOM,
  ...
  if (ESP_OK == nvs_set_i32(_ls_settings_nvs_handle,
                            LS_SETTINGS_NVS_KEY_SERVO_DELTA,
  ...
  if (ESP_OK == nvs_set_i32(_ls_settings_nvs_handle,
                            LS_SETTINGS_NVS_KEY_LIGHTSENSE_OFF,
  ...
  if (ESP_OK == nvs_set_i32(_ls_settings_nvs_handle,
                            LS_SETTINGS_NVS_KEY_LIGHTSENSE_ON,
  ...
  if (ESP_OK == nvs_set_i32(_ls_settings_nvs_handle,
                            LS_SETTINGS_NVS_KEY_SLEEP_LIGHT_ENABLE,
...
void ls_settings_set_mode(BaseType_t mode){
  _ls_settings_mode = mode;
}
BaseType_t ls_settings_get_mode(void){
  return _ls_settings_mode;
}
...
BaseType_t ls_settings_get_servo_top(void) { return _ls_settings_servo_top; }
...
BaseType_t ls_settings_map_control_to_servo_pulse_delta(BaseType_t adc) {
  adc = _make_log_response(
      _map(_constrain(adc, LS_CONTROLS_READING_BOTTOM, LS_CONTROLS_READING_TOP),
           LS_CONTROLS_READING_BOTTOM, LS_CONTROLS_READING_TOP, 0, 2047),
      11);
  return _map(adc, 0, 2047, LS_SERVO_DELTA_PER_TICK_MIN,
              LS_SERVO_DELTA_PER_TICK_MAX);
}
void ls_settings_set_servo_pulse_delta(BaseType_t microseconds_per_tick) {
  _ls_settings_servo_pulse_delta =
      _constrain(microseconds_per_tick, LS_SERVO_DELTA_PER_TICK_MIN,
                 LS_SERVO_DELTA_PER_TICK_MAX);
}
