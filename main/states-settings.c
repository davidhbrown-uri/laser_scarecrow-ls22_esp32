...
ls_State ls_state_settings_upper(ls_event event) {
  ...
  // somewhat awkward patch for #44
  // https://github.com/davidhbrown-uri/laser_scarecrow-ls22_esp32/issues/44
...
static enum ls_buzzer_scale _ls_state_settings_lightsense_scale[] = {
    ...
    LS_BUZZER_SCALE_Dx, LS_BUZZER_SCALE_E, LS_BUZZER_SCALE_F,
    LS_BUZZER_SCALE_Fx,
    LS_BUZZER_SCALE_G, // 4 (5th value) => center of range
    // 5..8 => switch at higher levels
    LS_BUZZER_SCALE_Gx, LS_BUZZER_SCALE_A, LS_BUZZER_SCALE_Ax,
    LS_BUZZER_SCALE_B};
...
ls_State ls_state_settings_both(ls_event event) {
  ...
  case LSEVT_STATE_ENTRY:
    ...
    if (_ls_state_settings_both_current_light_threshold_index(&index)) {
      ...
      for (int i = 0; i < LS_STATE_SETTINGS_LIGHTSENSE_TRILL_LIMIT; i++) {
        ls_buzzer_note(_ls_state_settings_lightsense_scale
                           [LS_LIGHTSENSE_THRESHOLD_DEFAULT],
                       pdMS_TO_TICKS(150));
        ls_buzzer_note(_ls_state_settings_lightsense_scale[index],
                       pdMS_TO_TICKS(150));
      }
    }
    break;
  ...
  case LSEVT_CONTROLS_SLIDER1: // light threshold
    control_value = *((BaseType_t *)event.value);
    // map input to index
    index = _map(_constrain(control_value, LS_CONTROLS_READING_BOTTOM,
                            LS_CONTROLS_READING_TOP),
                 LS_CONTROLS_READING_BOTTOM, LS_CONTROLS_READING_TOP, 0,
                 LS_LIGHTSENSE_THRESHOLDS_COUNT - 1);
    if (index != _ls_state_settings_lightsense_index) {
      ls_settings_set_light_threshold_on(ls_lightsense_threshold_on_mv(index));
      ls_settings_set_light_threshold_off(
          ls_lightsense_threshold_off_mv(index));
#ifdef LSDEBUG_SETTINGS
      ls_debug_printf("Setting lightsense thresholds to index %d: on at %dmV; "
                      "off at %dmV.\n",
                      index, ls_settings_get_light_threshold_on(),
    ...
    if (_ls_state_settings_lightsense_trills <
        LS_STATE_SETTINGS_LIGHTSENSE_TRILL_LIMIT) {
      _ls_state_settings_lightsense_trills++;
      ls_buzzer_note(
          _ls_state_settings_lightsense_scale[LS_LIGHTSENSE_THRESHOLD_DEFAULT],
          pdMS_TO_TICKS(50));
      ls_buzzer_note(_ls_state_settings_lightsense_scale[index],
                     pdMS_TO_TICKS(50));
    }
