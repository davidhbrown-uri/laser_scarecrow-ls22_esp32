...
#define VERSION_MESSAGE                                                        \
  "URI Laser Scarecrow Aquaculture Prototype version 3.0.0 (Fast Spinning)\n"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
 ...
 * Phase 4: setup state machine to handle events (read saved settings in poweron
...
void app_main(void) {
  ...
  xTaskCreate(&ls_stepper_task, "stepper", configMINIMAL_STACK_SIZE * 3, NULL,
              30, NULL);

  // high-priority; time/performance sensitive (20-29)
  xTaskCreate(&ls_controls_task, "controls_task", configMINIMAL_STACK_SIZE * 3,
              NULL, 20, NULL);

  // medium-priority (10-19)
  xTaskCreate(&event_handler_state_machine, "event_handler_state_machine",
              configMINIMAL_STACK_SIZE * 3, NULL, 15, NULL);

  // lowest priority (1-9)
  xTaskCreate(&ls_leds_handler_task, "leds_handler",
              configMINIMAL_STACK_SIZE * 2, NULL, 6, NULL);
  xTaskCreate(&ls_buzzer_handler_task, "buzzer_handler",
              configMINIMAL_STACK_SIZE * 2, NULL, 5, NULL);
  xTaskCreate(&ls_servo_task, "servo_task", configMINIMAL_STACK_SIZE * 3, NULL,
              3, NULL);
  //    xTaskCreate(&ls_coverage_task, "coverage_task", configMINIMAL_STACK_SIZE
  //    * 3, NULL, 2, NULL); // cannot do before map is ready
  xTaskCreate(&ls_tilt_task, "tilt_task", configMINIMAL_STACK_SIZE * 3, NULL, 2,
              NULL);
  xTaskCreate(&ls_lightsense_read_task, "lightsense_read",
              configMINIMAL_STACK_SIZE * 3, NULL, 1, NULL);

#ifdef LSDEBUG_TAPEMODE
  xTaskCreate(&ls_tapemode_debug_task, "tapemode_debug_task",
              configMINIMAL_STACK_SIZE * 3, NULL, 2, NULL);
#endif
#ifdef LSDEBUG_STEPPER
  xTaskCreate(&ls_stepper_debug_task, "stepper_debug",
              configMINIMAL_STACK_SIZE * 3, NULL, 2, NULL);
#endif
