...
static void _selftest_update_oled(void) {
  ...
  if (_selftest_light_high && _selftest_light_low) {
    ...
    ls_oled_println("Light: %c %c", _selftest_light_high ? ' ' : 0x01,
                    _selftest_light_low ? ' ' : 0x02);
  }
  if (_selftest_tilt_detect && _selftest_tilt_ok) {
    ls_oled_println("> Tilt OK");
  } else {
    ls_oled_println("Tilt: %c %c", _selftest_tilt_detect ? ' ' : '/',
                    _selftest_tilt_ok ? ' ' : '|');
  }
  ...
  if (_selftest_switches_off && _selftest_switches_upper &&
    ...
    ls_oled_println("Switches: %c%c%c%c", _selftest_switches_off ? ' ' : 'O',
                    _selftest_switches_upper ? ' ' : 'U',
                    _selftest_switches_lower ? ' ' : 'L',
                    _selftest_switches_both ? ' ' : 'B');
  }
  if (_selftest_slider1_min && _selftest_slider1_max && _selftest_slider2_min &&
      _selftest_slider2_max) {
    ls_oled_println("> Sliders OK");
  } else {
    ls_oled_println("Slider 1%c%c; 2%c%c", _selftest_slider1_min ? ' ' : 0x02,
...
void selftest_event_handler(ls_event event) {
  if (_selftest_wait_for_buzzer_warning_complete &&
  ...
  case LSEVT_STATE_ENTRY:
    ...
    ls_servo_jumpto(LS_SERVO_US_ASSEMBLY_REFERENCE);
  ...
  case LSEVT_CONTROLS_SLIDER1:
    ls_leds_single(0,
                   _constrain(_map(*event_value, LS_CONTROLS_READING_BOTTOM,
                                   LS_CONTROLS_READING_TOP, 0, 255),
                              0, 255) *
                           0x10000 |
                       _constrain(_map(*event_value, LS_CONTROLS_READING_BOTTOM,
                                       LS_CONTROLS_READING_TOP, 255, 0),
                                  0, 255) *
                           0x100);
    if (!ls_buzzer_in_use()) {
      ls_buzzer_tone(_map(*event_value, LS_CONTROLS_READING_BOTTOM,
                          LS_CONTROLS_READING_TOP, 1000, 2000));
    }
  ...
  case LSEVT_CONTROLS_SLIDER2:
    ...
    if (!ls_buzzer_in_use()) {
      ls_buzzer_tone(_map(*event_value, LS_CONTROLS_READING_BOTTOM,
                          LS_CONTROLS_READING_TOP, 2000, 4000));
    }
    if (*event_value >= LS_CONTROLS_READING_TOP) {
