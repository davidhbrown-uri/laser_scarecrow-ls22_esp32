...
static void _ls_buzzer_effect_alternate_high(int duration_ms) {
  ...
  for (int i = 0; i < pdMS_TO_TICKS(duration_ms); i += 2) {
    _ls_buzzer_frequency(
        3100); // resonant frequency of bucket piezo (Adafruit spec)
    vTaskDelay(1);
    _ls_buzzer_frequency(2900); // resonant frequency of external control piezo
                                // (PS1720P02 apx from graph)
    vTaskDelay(1);
  }
  ESP_ERROR_CHECK(ledc_stop(BUZZER_SPEED, BUZZER_CHANNEL, 0));
}
...
void ls_buzzer_handler_task(void *pvParameter) {
  ...
  while (1) {
    if (xQueueReceive(ls_buzzer_queue, &received, portMAX_DELAY) != pdTRUE) {
#ifdef LSDEBUG_BUZZER
      ls_debug_printf(
          "No buzz requested maximum delay... getting very bored.\n");
#endif
    } else {
      ...
      default:;
#ifdef LSDEBUG_BUZZER
        ls_debug_printf("Unknown ls_buzzer_effect %d -- I'm confused",
                        received.effect);
#endif
      }
    }
