...
void ls_tapemode_init(void) { _ls_tapemode = ls_tapemode_current(); }

enum ls_tapemode_mode ls_tapemode(void) { return _ls_tapemode; }
    ...
    if (current != previous) {
      if (stable_count >= 15) {
        ...
        case LS_TAPEMODE_LIGHT_SAFE:
          ...
          break;
        default:; // don't play anything on return to selftest; we know that
...
void ls_tapemode_debug_task(void *pvParameter) {
  while (1) {
    ...
    printf("Tape mode %d (raw: %d)\n", (int)ls_tapemode_from_adc(adc_reading),
           adc_reading);
