...
void ls_laser_pulse_init(void) {
  gpio_set_level(LSGPIO_LASERPOWERENABLE,
                 0); // don't do anything while we get ready
  ...
  ESP_ERROR_CHECK(
      timer_init(TIMER_GROUP_0, TIMER_1, &laser_pulse_timer_config));
  ESP_ERROR_CHECK(timer_set_counter_value(TIMER_GROUP_0, TIMER_1, 0ULL));
  // alarm every 25ms
  ESP_ERROR_CHECK(
      timer_set_alarm_value(TIMER_GROUP_0, TIMER_1, APB_CLK_FREQ / 40000 / 40));
  ESP_ERROR_CHECK(timer_enable_intr(TIMER_GROUP_0, TIMER_1));
  ESP_ERROR_CHECK(timer_isr_callback_add(TIMER_GROUP_0, TIMER_1,
                                         ls_laser_pulse_isr_callback, NULL, 0));
