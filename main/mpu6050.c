...
static esp_err_t mpu6050_write_reg_byte(uint8_t register_number, uint8_t data) {
  // see page 35 of MPU-6000/MPU-6050 Product Specification; single-byte write
  // sequence
  i2c_cmd_handle_t cmd = i2c_cmd_link_create();
  i2c_master_start(cmd); // S
  i2c_master_write_byte(cmd, MPU6050_ADDR << 1 | I2C_MASTER_WRITE,
                        ACK_CHECK_EN);                       // AD+W (ACK)
...
static esp_err_t mpu6050_read_reg_uint8(uint8_t register_number,
                                        uint8_t *data) {
  // see page 36 of MPU-6000/MPU-6050 Product Specification; single-byte read
  // sequence
  i2c_cmd_handle_t cmd = i2c_cmd_link_create();
  i2c_master_start(cmd); // S
  i2c_master_write_byte(cmd, MPU6050_ADDR << 1 | I2C_MASTER_WRITE,
  ...
  i2c_master_write_byte(cmd, register_number, ACK_CHECK_EN); // RA (ACK)
  i2c_master_start(cmd);                                     // S
  i2c_master_write_byte(cmd, MPU6050_ADDR << 1 | I2C_MASTER_READ,
                        ACK_CHECK_EN);              // AD+R (ACK)
...
static esp_err_t mpu6050_read_reg_int16(uint8_t register_number,
                                        int16_t *data) {
 ...
 * @todone Figure out why every other time the ESP32 is reset, this device is
...
void mpu6050_begin(void) {
  ...
  // page 41 of the MPU60x0 register map document has a note that seemed helpful
  // with the zero value reporting: When using SPI interface, user should use
  // DEVICE_RESET (register 107) as well as SIGNAL_PATH_RESET (register 104) to
  // ensure the reset is performed properly. The sequence used should be:
  // 1. Set DEVICE_RESET = 1 (register PWR_MGMT_1)
  // 2. Wait 100ms
  // 3. Set GYRO_RESET = ACCEL_RESET = TEMP_RESET = 1 (register
  // SIGNAL_PATH_RESET)
  // 4. Wait 100ms
  // just the primary device reset + delay, not the signal path reset appears to
  // be sufficient
  ESP_ERROR_CHECK(
      mpu6050_write_reg_byte(MPU6050_PWR_MGMT_1, MPU6050_PWR_MGMT_1_RESET_BIT));
  // wait 100ms
  vTaskDelay(pdMS_TO_TICKS(100));
  ESP_ERROR_CHECK(
      mpu6050_write_reg_byte(MPU6050_PWR_MGMT_1, MPU6050_PWR_MGMT_1_CYCLE_BIT));
  ESP_ERROR_CHECK(
      mpu6050_write_reg_byte(MPU6050_PWR_MGMT_2, MPU6050_PWR_MGMT_2_ZA_ONLY));
}
