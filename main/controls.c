...
static enum ls_controls_status ls_controls_current_status =
...
int _ls_controls_get_averaged_adc_mv(
    adc2_channel_t controls_channel, adc_atten_t controls_atten,
    esp_adc_cal_characteristics_t *controls_adc_cal) {
  int adc_reading = 0;
  uint32_t adc_sum = 0;
  ...
  for (int i = 0; i < LS_CONTROLS_READINGS_TO_AVERAGE; i++) {
    ESP_ERROR_CHECK(
        adc2_get_raw(controls_channel, ADC_WIDTH_12Bit, &adc_reading));
    adc_sum += (uint32_t)adc_reading;
  }
  xSemaphoreGive(adc2_mux);
  return esp_adc_cal_raw_to_voltage(
      (int)(adc_sum / LS_CONTROLS_READINGS_TO_AVERAGE), controls_adc_cal);
}
...
#define LS_CONTROLS_TASK_SWITCH_READINGS 3
  ...
  adc2_channel_t controls_channels[] = {LSADC2_SWITCHES, LSADC2_SLIDER1,
                                        LSADC2_SLIDER2};
  adc_atten_t controls_atten[] = {LSADCATTEN_SWITCHES, LSADCATTEN_SLIDER,
                                  LSADCATTEN_SLIDER};
  esp_adc_cal_characteristics_t
      controls_adc_cal[LS_CONTROLS_TASK_CONTROLS_COUNT];
  ...
  for (int i = 0; i < LS_CONTROLS_TASK_SWITCH_READINGS; i++) {
    ...
    esp_adc_cal_characterize(ADC_UNIT_1, controls_atten[i], ADC_WIDTH_12Bit,
                             1100, &controls_adc_cal[i]);
  }

  while (1) {
    // read controls
    for (int control_number = 0;
         control_number < LS_CONTROLS_TASK_CONTROLS_COUNT; control_number++) {
      controls_readings[control_number] = _ls_controls_get_averaged_adc_mv(
          controls_channels[control_number], controls_atten[control_number],
          &controls_adc_cal[control_number]);
    }

#ifdef LSDEBUG_CONTROLS_VERBOSE
    ls_debug_printf("Controls:  switches=%dmV slider1=%dmV slider2=%dmV\n",
                    controls_readings[0], controls_readings[1],
                    controls_readings[2]);
#endif
    ...
    for (int i = 1; i < LS_CONTROLS_TASK_SWITCH_READINGS; i++) {
      switch_readings_match =
          switch_readings_match && switch_readings[i] == switch_readings[0];
    }
    if (switch_readings_match &&
      ...
      xQueueSendToBack(ls_event_queue, (void *)&switch_event,
                       pdMS_TO_TICKS(1000));
    } // if switches changed

    if (ls_controls_get_current_status() != LS_CONTROLS_STATUS_OFF) {
      if (moved_control[1] ||
          _difference_exceeds_threshold(_ls_controls_current_slider1,
                                        controls_readings[1],
                                        LS_CONTROLS_READING_MOVE_THRESHOLD)) {
        if (!moved_control[1]) {
      ...
      if (moved_control[2] ||
          _difference_exceeds_threshold(_ls_controls_current_slider2,
                                        controls_readings[2],
                                        LS_CONTROLS_READING_MOVE_THRESHOLD)) {
        if (!moved_control[2]) {
          fastreads = LS_CONTROLS_FASTREADS_AFTER_MOVE;
    ...
    vTaskDelay(ls_controls_current_status == LS_CONTROLS_STATUS_OFF
                   ? pdMS_TO_TICKS(600)
                   : pdMS_TO_TICKS(100));
  } // while 1
} // ls_controls_task