...
#define LSGPIO_OUTPUT_PIN_SEL                                                  \
  ((1ULL << LSGPIO_LASERPOWERENABLE) | (1ULL << LSGPIO_LASERHEATERENABLE) |    \
   (1ULL << LSGPIO_SERVOPOWERENABLE) | (1ULL << LSGPIO_REFLECTANCEENABLE) |    \
   (1ULL << LSGPIO_STEPPERENABLE) | (1ULL << LSGPIO_STEPPERSTEP) |             \
   (1ULL << LSGPIO_STEPPERDIRECTION) | (1ULL << LSGPIO_BUZZERENABLE))

#define LSGPIO_INPUT_PIN_SEL                                                   \
  ((1ULL << LSGPIO_SLIDER1) | (1ULL << LSGPIO_SLIDER2) |                       \
   (1ULL << LSGPIO_SWITCHES) | (1ULL << LSGPIO_LASERTEMP) |                    \
   (1ULL << LSGPIO_LIGHTSENSE) | (1ULL << LSGPIO_MAGNETSENSE) |                \
   (1ULL << LSGPIO_REFLECTANCESENSE) | (1ULL << LSGPIO_TAPESETTING))
...
void ls_gpio_initialize(void) {
  ...
  gpio_set_level(LSGPIO_STEPPERENABLE,
                 STEPPERENABLE_DISABLE); // TMS2209 Enable is active low
...
void check_efuse(void) {
  ...
  if (esp_adc_cal_check_efuse(ESP_ADC_CAL_VAL_EFUSE_TP) == ESP_OK) {
    ...
    printf("Cannot retrieve eFuse Two Point calibration values. Default "
