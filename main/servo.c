/*
    ...
    Copyright (C) 2022-2024 Isaac Chen and David H. Brown
...
static bool _ls_servo_is_on =
    false; // The servo will start off from ls_gpio_initialize()
...
static void _ls_servo_jump_to_pw(uint32_t pulse_width) {
  mcpwm_set_duty_in_us(LS_SERVO_MCPWM_UNIT, LS_SERVO_MCPWM_TIMER,
                       LS_SERVO_MCPWM_GENERATOR, pulse_width);
}
...
void ls_servo_init() {
  ...
  mcpwm_gpio_init(LS_SERVO_MCPWM_UNIT, LS_SERVO_MCPWM_IO_SIGNALS,
                  LSGPIO_SERVOPULSE);
  mcpwm_config_t pwm_config = {.frequency =
                                   50, // Frequency = 50Hz, i.e. for every servo
                                       // motor time period should be 20ms
                               .cmpr_a = 0, // Duty cycle of PWMxA = 0
                               .counter_mode = MCPWM_UP_COUNTER,
                               .duty_mode = MCPWM_DUTY_MODE_0};
  mcpwm_init(LS_SERVO_MCPWM_UNIT, LS_SERVO_MCPWM_TIMER,
...
// Creates and sends a message to the servo task to put the servo in random
// motion mode
...
// Creates and sends a message to the servo task to move the servo smoothly to a
// specified position
...
// Creates and sends a message to the servo task to immediately jump the servo
...
void ls_servo_task(void *pvParameter) {
  ...
  uint16_t current_pulse_width = LS_SERVO_US_MIN;
  uint16_t target_pulse_width = LS_SERVO_US_MIN;
  ...
  while (1) {
    ...
    bool servo_should_move =
        _ls_servo_is_on && (current_pulse_width != target_pulse_width ||
...
#ifdef LSDEBUG_SERVO
        ls_debug_printf(
            "Servo task received off event, feeling... sleepy.....\n");
#endif
        _ls_servo_off();
        break;
...
#ifdef LSDEBUG_SERVO
        ls_debug_printf(
            "Servo task received jump_to event with pulse width %d\n",
            received.data);
#endif
...
#ifdef LSDEBUG_SERVO
        ls_debug_printf(
            "Servo task received move_to event with pulse width %d\n",
            received.data);
#endif
        // If the servo is currently off, turn it on
        _ls_servo_on();
...
#ifdef LSDEBUG_SERVO
        ls_debug_printf("Servo task received move_randomly event, entering "
                        "random motion mode\n");
#endif
        // If the servo is currently off, turn it on
        _ls_servo_on();
...
#ifdef LSDEBUG_SERVO
        ls_debug_printf(
            "Servo task received sweep event, entering sweep mode\n");
#endif
        // If the servo is currently off, turn it on
        _ls_servo_on();
    ...
    // again.
      ...
      if (mode == LS_SERVO_MODE_RANDOM &&
...
#endif
      } else if (mode == LS_SERVO_MODE_SWEEP &&
                 current_pulse_width == target_pulse_width) {
...
#endif
        // If the current pulse width is at the top, queue a
        // LSEVT_SERVO_SWEEP_TOP event
        ...
        // If the current pulse width is at the bottom, queue a
        // LSEVT_SERVO_SWEEP_BOTTOM event
        ...
        // Most of the time, it will be at either end - but possibly not if we
        // just entered sweep mode from another In that case, move towards
        // whichever end is currently further away
        target_pulse_width = current_pulse_width < mid ? bottom : top;
        // pause when target reached
#ifdef LSDEBUG_SERVO
        ls_debug_printf("New servo target: %d after %dms pause\n",
                        target_pulse_width,
                        ls_settings_get_servo_sweep_pause_ms());
#endif
      ...
      current_pulse_width = (uint16_t)_constrain(
          ...
          (BaseType_t)current_pulse_width +
