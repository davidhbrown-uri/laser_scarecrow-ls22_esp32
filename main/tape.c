...
bool ls_tape_sensor_is_enabled(void) { return _ls_tape_sensor_enabled; }
...
void ls_tape_sensor_selftest_task(void *pvParameter) {
  ...
  while (1) {
    ...
    if (tape_reading < LS_REFLECTANCE_ADC_MAX_LIGHT &&
        last_tape_reading >= LS_REFLECTANCE_ADC_MAX_LIGHT) {
      ls_buzzer_effect(LS_BUZZER_PLAY_ROOT);
      ls_event_enqueue(LSEVT_SELFTEST_TAPE_LIGHT);
    }
    if (tape_reading > LS_REFLECTANCE_ADC_MIN_DARK &&
        last_tape_reading <= LS_REFLECTANCE_ADC_MIN_DARK) {
