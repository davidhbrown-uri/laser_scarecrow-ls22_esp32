...
void ls_event_enqueue_noop(void) { ls_event_enqueue(LSEVT_NOOP); }
...
/**
 * Insert an event of the specified type at the front of the queue; value is
 * NULL
 */
...
void ls_event_empty_queue(void) { xQueueReset(ls_event_queue); }