...
void _ls_state_rehome_timer_callback(TimerHandle_t xTimer) {
  ...
  event.value = NULL;
  if (xQueueSendToBack(ls_event_queue, (void *)&event, pdMS_TO_TICKS(5000)) !=
      pdPASS) {
#ifdef LSDEBUG_STATES
    ls_debug_printf("WARNING: Could not enqueue LSEVT_REHOME_REQUIRED; will "
...
void ls_state_init(void) {
  _ls_state_rehome_timer = xTimerCreate(
      "rehome_timer",                                 // pcTimerName
...
void event_handler_state_machine(void *pvParameter) {
  ...
  // if the ls_state_current is set prior to beginning this task, start it up
  // with an entry event
  ...
  while (1) {
    if (xQueueReceive(ls_event_queue, &event,
                      pdMS_TO_TICKS(LS_EVENT_NOOP_TIMEOUT_MS)) != pdTRUE) {
      ...
      if (ls_state_current.func != NULL) // we have a current state
      {
        ...
        previous_state.func =
            ls_state_current
                .func; // save current state as previous to next event
      } else {
#ifdef LSDEBUG_STATES
...
#ifdef LSDEBUG_STATES
        ls_debug_printf(
            "State poweron (LS_MAP_STATUS_OK)=> pre-laser warning\n");
#endif
        successor.func = ls_state_prelaserwarn;
      } else {
#ifdef LSDEBUG_STATES
...
static bool _ls_state_prelaserwarn_magnet_enter,
    _ls_state_prelaserwarn_magnet_leave;
...
ls_State ls_state_prelaserwarn(ls_event event) {
  ...
  case LSEVT_STATE_ENTRY:
    ...
    ls_stepper_set_maximum_steps_per_second(
        LS_STEPPER_STEPS_PER_SECOND_WARNING);
  ...
  if (_ls_state_prelaserwarn_buzzer_complete &&
      _ls_state_prelaserwarn_movement_complete) {
    ls_leds_off();
    if (NULL == _ls_state_prelaserwarn_successor) {
      ls_state_set_prelaserwarn_successor(ls_state_active); // default
    }
    if (!(_ls_state_prelaserwarn_magnet_enter &&
          _ls_state_prelaserwarn_magnet_leave)) {
...
ls_State ls_state_active(ls_event event) {
  ...
  case LSEVT_STATE_ENTRY:
    ...
    if (ls_map_get_status() == LS_MAP_STATUS_OK) {
      ...
      xTimerReset(_ls_state_rehome_timer, pdMS_TO_TICKS(5000));
      xTaskCreate(&ls_coverage_task, "coverage_task",
                  configMINIMAL_STACK_SIZE * 3, NULL, 2,
                  &ls_coverage_task_handle); // cannot do before map is ready
    } else {
  ...
  if (successor.func != ls_state_active) {
    if (NULL != ls_coverage_task_handle) {
      ...
      ls_coverage_task_handle =
          NULL; // probably not necessary now, but just in case
    }
    ls_stepper_stop();
    ls_servo_off();
    ls_laser_set_mode_off();
...
ls_State ls_state_sleep(ls_event event) {
  ...
  case LSEVT_STATE_ENTRY:
    ...
    break;
  case LSEVT_STEPPER_FINISHED_MOVE:
    // the magnet sensor includes an LED which could pointlessly drain power
    // during sleep yes, wind might blow the arm back to the magnet, but at
    // least we tried!
...
static int _ls_state_map_enable_count = 0, _ls_state_map_disable_count = 0,
           _ls_state_map_misread_count = 0;
static enum ls_buzzer_effects _ls_state_map_fail_reason_tune =
    LS_BUZZER_PLAY_NOTHING;
ls_State ls_state_map_build(ls_event event) {
  ...
  case LSEVT_STATE_ENTRY:
    _ls_state_map_build_steps_remaining =
        LS_STEPPER_STEPS_PER_ROTATION / LS_MAP_RESOLUTION;
    ls_tape_sensor_enable();
    while (ls_buzzer_in_use() || ls_stepper_is_moving()) {
      // if we start moving while the buzzer is still indicating successfull
      // homing, we get a bunch of pitches queued that play faster than the
      // others
      vTaskDelay(1);
    }
    ls_event_empty_queue(); // in case of a trailing LSEVT_STEPPER_FINISHED_MOVE
    ls_stepper_set_maximum_steps_per_second(
        LS_STEPPER_STEPS_PER_SECOND_MAPPING);
  ...
  case LSEVT_STEPPER_FINISHED_MOVE:
    ...
    if (_ls_state_map_build_steps_remaining > 0) {
      _ls_state_map_build_read_and_set_map(&_ls_state_map_enable_count,
      ...
      ls_stepper_forward(LS_MAP_RESOLUTION);
      _ls_state_map_build_steps_remaining--;
    } else { // we're done building the map
      bool badmap = false;
      int low_peak_bin, high_peak_bin, low_edge_bin, high_edge_bin;
      _ls_state_map_build_histogram(ls_map_min_adc(), ls_map_max_adc());
      _ls_state_map_build_histogram_get_peaks_edges(
          &low_peak_bin, &low_edge_bin, &high_peak_bin, &high_edge_bin);
      if (low_edge_bin >= high_edge_bin) // mapping failed; insufficient
                                         // contrast (probably no tape)
      {
        badmap = true;
      } else // reset the map based on customized histogram
      {
        // rebuild the histogram just between the peaks
        uint16_t low_peak_adc =
            _map(low_peak_bin, 0, LS_MAP_HISTOGRAM_BINCOUNT - 1,
                 ls_map_min_adc(), ls_map_max_adc());
        uint16_t high_peak_adc =
            _map(high_peak_bin, 0, LS_MAP_HISTOGRAM_BINCOUNT - 1,
                 ls_map_min_adc(), ls_map_max_adc());
        _ls_state_map_build_histogram(low_peak_adc, high_peak_adc);
        _ls_state_map_build_histogram_get_peaks_edges(
            &low_peak_bin, &low_edge_bin, &high_peak_bin, &high_edge_bin);
        // remap to custom thresholds
        _ls_state_map_build_set_map(
            &_ls_state_map_enable_count, &_ls_state_map_disable_count,
            &_ls_state_map_misread_count,
            _map(low_edge_bin, 0, LS_MAP_HISTOGRAM_BINCOUNT - 1, low_peak_adc,
                 high_peak_adc), // low threshold
            _map(high_edge_bin, 0, LS_MAP_HISTOGRAM_BINCOUNT - 1, low_peak_adc,
                 high_peak_adc) // high threshold
        );
      }
#ifdef LSDEBUG_MAP
      ls_debug_printf(
      ...
      if (0 == _ls_state_map_enable_count || 0 == _ls_state_map_disable_count) {
        badmap = true;
#ifdef LSDEBUG_MAP
        ls_debug_printf("\nBad map: must include at least one enabled and one "
                        "disabled reading.\n");
#endif
        switch (ls_tapemode()) {
        case LS_TAPEMODE_DARK:
        case LS_TAPEMODE_DARK_SAFE:
          _ls_state_map_fail_reason_tune = (0 == _ls_state_map_enable_count)
                                               ? LS_BUZZER_PLAY_TAPE_DISABLE
                                               : LS_BUZZER_PLAY_TAPE_ENABLE;
          break;
      ...
      if (badmap) {
        ...
        ls_debug_printf("Set LS_MAP_STATUS_OK; random map span strategy has %d "
                        "steps in total.\n",
                        total);
#else
        ls_map_find_spans();
...
/** "map" is/was how I think of it, but "scanning" is more understandable to
 * growers, I think*/
...
#ifdef LSDEBUG_STATES
  ls_debug_printf(
      "LSEVT_NOOP enqueued by returning ls_state_error_scanning<<<\n");
#endif
  return successor;
}
...
ls_State ls_state_error_tilt(ls_event event) {
  ...
  default:
    ...
    if (pdFALSE == xQueuePeek(ls_event_queue, &event,
                              pdMS_TO_TICKS(LS_FAILURE_LEDS_OFF_AFTER))) {
      ls_leds_off();
    }
    if (pdFALSE == xQueuePeek(ls_event_queue, &event,
                              pdMS_TO_TICKS(LS_FAILURE_REPEAT_INTERVAL))) {
      ls_event_enqueue_noop();
    }
  }
  return successor;
...
/**
 * @brief If no accelerometer is found, play a warning tone then attempt to
 * restart MCU
 *
 * @param event
 * @return ls_State
