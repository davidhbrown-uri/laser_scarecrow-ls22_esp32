/*
    Control software for URI Laser Scarecrow, 2022-2024 Models
 ...
 *   a) if never homed or if position offset > threshold, repeat and set to
 * average b) if position offset <= threshold, do nothing... stepper is
 * generally more accurate than magnet
 */
...
enum {
  ...
  LS_HOME_SUBSTATE_WAIT_FOR_NO_STEPS, // 5 -- use only if there may not be a end
                                      // move event, e.g., on init which could
                                      // be poweron/wake
...
/**
 * May be called by previous state... active does not require homing; power-on,
 * wake-up do (when tape map in use).
 */
void ls_substate_home_require_rehome() { _ls_home_homing_required = true; }
...
static void _ls_substate_home_backup(ls_event event) {
  ...
  case LSEVT_SUBSTATE_ENTRY: {
    ls_stepper_position_t magnet_position =
        _ls_substate_magnet_entry_offset < 0
            ? _ls_substate_magnet_entry_offset + LS_STEPPER_STEPS_PER_ROTATION
            : _ls_substate_magnet_entry_offset;
    ...
    ls_debug_printf("Need to back up at least %d steps from stopped position "
                    "%d to magnet entry at %d.\n",
                    steps, ls_stepper_get_position(), magnet_position);
#endif
    ls_stepper_set_maximum_steps_per_second(
        LS_HOME_INITIAL_STEPPER_STEPS_PER_SECOND);
...
#endif
    ls_stepper_stop();
    /* if offset exceeds threshold, we will need to reset home position (might
     * already be doig that; it's okay to set true twice!)*/
    if ((int)event.value > LS_HOME_OFFSET_THRESHOLD_TO_REHOME ||
        -((int)event.value) > LS_HOME_OFFSET_THRESHOLD_TO_REHOME) {
  ...
  case LSEVT_STEPPER_FINISHED_MOVE:
    if (_ls_home_found_magnet) {
      if (_ls_home_homing_required) {
        ...
        _ls_home_homing_offsets[_ls_home_homing_index] =
            _ls_substate_magnet_entry_offset;
#ifdef LSDEBUG_HOMING
      ...
      else { // full rehoming not required
        _ls_substate_home_substate_phase = LS_HOME_SUBSTATE_COMPLETE;
        ls_event_enqueue(LSEVT_SUBSTATE_ENTRY);
      }
    }    // if magnet found
...
#ifdef LSDEBUG_HOMING
      ls_debug_printf(
          "Slow step to magnet failed. Move ended at position %d.\n",
...
static void _ls_substate_home_rotate_to_magnet(ls_event event) {
  ...
  case LSEVT_SUBSTATE_ENTRY:
    ls_stepper_set_maximum_steps_per_second(
        LS_HOME_INITIAL_STEPPER_STEPS_PER_SECOND);
    ls_stepper_forward(LS_STEPPER_STEPS_PER_ROTATION *
  ...
  case LSEVT_MAGNET_ENTER:
    ...
    if (ls_stepper_get_steps_taken() >
        ((5 * LS_STEPPER_STEPS_PER_ROTATION) / 4)) {
      ls_debug_printf("WARNING: took > %d rotation to find magnet!\n",
                      ls_stepper_get_steps_taken() /
                          LS_STEPPER_STEPS_PER_ROTATION);
    }
#endif
    break;
  case LSEVT_STEPPER_FINISHED_MOVE:
    if (_ls_home_found_magnet) {
      ls_leds_cycle(LEDCYCLE_HOMING); // if a subsequent try might have been
                                      // showing warning.
      _ls_substate_home_substate_phase = LS_HOME_SUBSTATE_BACKUP;
      ls_event_enqueue(LSEVT_SUBSTATE_ENTRY);
    } // if found magnet
