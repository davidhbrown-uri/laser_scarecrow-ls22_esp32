...
// see
// https://docs.espressif.com/projects/esp-idf/en/v4.4/esp32/api-reference/peripherals/timer.html
...
// Per ch 18 of the ESP32 Technical Reference manual, the minimum clock divisor
// is 2 So with the clock divider at 2, we'd need timer values of
//  - 160 (minimum) to meet A4988's STEP minimum, HIGH pulse width (LOW is the
//  same)
//  - 50,000 for the fastest step
//  - 5,500,000 for the slowest step
// The timers are 64-bit, so counting this number of steps should not be an
// issue Rather than aiming for 1ms pulses, toggling at the total timer count
// for a square(ish) wave would make sense.
...
enum ls_stepper_direction_t IRAM_ATTR ls_stepper_direction =
    LS_STEPPER_ACTION_FORWARD_STEPS;
static bool _ls_stepper_enable_skipping = false;
static int _ls_stepper_steps_per_second_max =
    LS_STEPPER_STEPS_PER_SECOND_DEFAULT;
...
static uint8_t _ls_stepper_random_reverse_per255 =
    LS_STEPPER_MOVEMENT_REVERSE_PER255;

// how many steps it will take to decelerate from full speed
static int _ls_stepper_steps_to_decelerate(int current_rate) {
  // used Excel to fit a quadratic curve (y=ax^2+bx+c) the the calculated
  // deceleration steps the constant term might want to change if the speed
  // limit is changed
  return (current_rate * current_rate) /
             (2 *
              LS_STEPPER_MOVEMENT_STEPS_DELTA_PER_SECOND) + // 'a' coefficient
                                                            // on x^2 is the
                                                            // reciprocal of 2x
                                                            // the steps delta
         (current_rate / 20) + // the 'b' coefficient is consistently 0.05...
                               // not sure exactly why
         10; // 'c' constant term for steps left over after steps delta has been
             // removed each time (3600 is not divisible by 800)
}

ls_stepper_position_t
ls_stepper_position_constrained(ls_stepper_position_t position) {
...
void ls_stepper_set_maximum_steps_per_second(int steps_per_second) {
  ...
  if (changed) {
    printf("Stepper speed set to %d max steps/s (%d requested); %d steps to "
           "decelerate.\n",
           _ls_stepper_steps_per_second_max, steps_per_second,
           _ls_stepper_steps_to_decelerate(_ls_stepper_steps_per_second_max));
  }
#endif
}
...
static bool IRAM_ATTR ls_stepper_step_isr_callback(void *args) {
  ...
  if (ls_stepper_steps_remaining > 0) // only do a step if any remain
  {
    ...
    if (0 == _ls_stepperstep_phase) { // beginning a step pulse (high)
      ...
      if (ls_laser_mode_is_mappped()) {
        gpio_set_level(LSGPIO_LASERPOWERENABLE,
                       ls_map_is_enabled_at(ls_stepper_position));
      }
    } else { // ending the step pulse (low)
...
void ls_stepper_init(void) {
  ...
  gpio_set_level(LSGPIO_STEPPERENABLE,
                 STEPPERENABLE_DISABLE); // don't do anything while we get ready
  ...
  ESP_ERROR_CHECK(
      timer_init(TIMER_GROUP_0, TIMER_0, &stepper_step_timer_config));
  ESP_ERROR_CHECK(timer_set_counter_value(TIMER_GROUP_0, TIMER_0, 0ULL));
  ESP_ERROR_CHECK(
      timer_set_alarm_value(TIMER_GROUP_0, TIMER_0,
                            APB_CLK_FREQ / LS_STEPPER_TIMER_DIVIDER /
                                LS_STEPPER_STEPS_PER_SECOND_MIN));
  ESP_ERROR_CHECK(timer_enable_intr(TIMER_GROUP_0, TIMER_0));
  ESP_ERROR_CHECK(timer_isr_callback_add(
      TIMER_GROUP_0, TIMER_0, ls_stepper_step_isr_callback, NULL, 0));
  ESP_ERROR_CHECK(timer_start(TIMER_GROUP_0, TIMER_0));
}

static void _ls_stepper_set_speed(void) {
  // if doing random movements and outside a span, allow faster movement
  // (skipping)
  if (_ls_stepper_enable_skipping) {
    ls_stepper_set_maximum_steps_per_second(
        ls_map_is_enabled_at(ls_stepper_position)
            ? _ls_stepper_speed_not_skipping
            : _ls_stepper_speed_when_skipping);
  }
  int steps_to_decelerate =
      _ls_stepper_steps_to_decelerate(_ls_stepper_speed_current_rate);

  bool could_accelerate =
      (int)ls_stepper_steps_remaining > steps_to_decelerate &&
      _ls_stepper_speed_current_rate < _ls_stepper_steps_per_second_max;
  bool should_decelerate =
      (int)ls_stepper_steps_remaining < steps_to_decelerate ||
      _ls_stepper_speed_current_rate > _ls_stepper_steps_per_second_max;
  ...
  ls_debug_printf("Current rate/max: %d/%d; steps to decelerate: %d; steps "
                  "remaining: %d; %c %c\n",
                  _ls_stepper_speed_current_rate,
                  _ls_stepper_steps_per_second_max, steps_to_decelerate,
                  ls_stepper_steps_remaining, could_accelerate ? '+' : ' ',
                  should_decelerate ? '-' : ' ');
#endif
  // but stay within bounds
  _ls_stepper_speed_current_rate = _constrain(_ls_stepper_speed_current_rate,
                                              LS_STEPPER_STEPS_PER_SECOND_MIN,
                                              _ls_stepper_steps_per_second_max);
...
void ls_stepper_task(void *pvParameter) {
  ...
  while (1) {
    if (uxQueueMessagesWaiting(ls_stepper_queue) > 0) {
      // peek at message queue because we need to stop if we have to change
      // direction first
      bool success = xQueuePeek(ls_stepper_queue, &message, 0);
      if (ls_stepper_steps_remaining > 0 && success &&
          ((LS_STEPPER_ACTION_FORWARD_STEPS == message.action &&
            LS_STEPPER_DIRECTION_REVERSE == ls_stepper_direction) ||
           (LS_STEPPER_ACTION_REVERSE_STEPS == message.action &&
            LS_STEPPER_DIRECTION_FORWARD == ls_stepper_direction))) {
#ifdef LSDEBUG_STEPPER
        xSemaphoreTake(print_mux, portMAX_DELAY);
        printf("Stepper stopping before change in direction (dir=%d; upcoming "
               "action=%d)\n",
               ls_stepper_direction, message.action);
        xSemaphoreGive(print_mux);
#endif
        current_action = LS_STEPPER_ACTION_STOP;
      } else { // dequeue the message
...
#endif
      gpio_set_level(LSGPIO_STEPPERENABLE,
                     STEPPERENABLE_ENABLE); // enabled because we need to
                                            // control deceleration
      ls_stepper_steps_remaining = _constrain(
          ls_stepper_steps_remaining, 0,
          _ls_stepper_steps_to_decelerate(_ls_stepper_speed_current_rate));
      _ls_stepper_set_speed();
      if (ls_stepper_steps_remaining <= 0) {
...
void IRAM_ATTR ls_stepper_set_home_position(void) { ls_stepper_position = 0; }
...
bool ls_stepper_is_stopped(void) { return 0 == ls_stepper_steps_remaining; }

BaseType_t ls_stepper_get_steps_taken(void) { return ls_stepper_steps_taken; }
...
void ls_stepper_debug_task(void *pvParameter) {
  while (1) {
    ls_debug_printf("STEPPER DEBUG: position=%d; remaining=%d; taken=%d; "
                    "direction=%d, step_phase=%d\n",
                    ls_stepper_position, ls_stepper_steps_remaining,
                    ls_stepper_steps_taken, ls_stepper_direction,
                    _ls_stepperstep_phase);
...
void ls_stepper_random_strategy_default(struct ls_stepper_move_t *move) {
  ...
  move->direction = ((uint8_t)random & 0xFF) > _ls_stepper_random_reverse_per255
                        ? false
                        : true;
  move->steps =
      LS_STEPPER_MOVEMENT_STEPS_MIN +
      ((random >> 16) *
       (ls_settings_get_stepper_random_max() - LS_STEPPER_MOVEMENT_STEPS_MIN) /
       65536);
#ifdef LSDEBUG_STEPPER_RANDOM
  ls_debug_printf(
      "Default strategy: At end of random move; moving randomly %s%d\n",
