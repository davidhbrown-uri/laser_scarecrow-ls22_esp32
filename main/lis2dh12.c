...
void lis2dh12_begin(void) {
  ...
  ls_i2c_write_reg_byte(LS_I2C_ADDRESS_LIS2DH12, LIS2DH_REGISTER_CTRL_REG1,
                        LIS2DH_CTRL_REG1_ODR10Hz | LIS2DH_CTRL_REG1_LPEN |
                            LIS2DH_CTRL_REG1_ZEN);
  // enable temperature
  //    ls_i2c_write_reg_byte(LS_I2C_ADDRESS_LIS2DH12, LIS2DH_REGISTER_CFG_REG,
  //    LIS2DH_CFG_REG_TEMP_EN);
}
float lis2dh120_read_accel_z(void) {
  // using low-resolution (8-bit signed)
  uint8_t raw = 0;
  ESP_ERROR_CHECK(ls_i2c_read_reg_uint8(LS_I2C_ADDRESS_LIS2DH12,
                                        LIS2DH_REGISTER_OUT_Z_H, &raw));
